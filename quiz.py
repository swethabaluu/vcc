...
def fetch_questions():
    """Fetch all questions from the MongoDB database."""
    return list(questions_collection.find({}, {'_id': 0}))  # Exclude the _id field
...
def quiz_app():
    ...
    for i, question_data in enumerate(questions):
        ...
        selected_options = st.multiselect("Select your answer:", options, key=i)
        user_answers[i] = selected_options

        # Submit Answer button
        if st.button(f"Submit Answer for Question {i+1}", key=f"submit{i}"):
            if len(selected_options) == 1 and check_answer(selected_options[0], question_data['answer']):
                st.success("Correct!")
                score += 1
            else:
